...
def payback_duration_years(df: pd.DataFrame) -> Optional[float]:
    ...
    for i in range(1, len(cum)):
        ...
        if a < 0 <= b:
            frac = (0 - a) / (b - a + 1e-12)
            # crossing happens within year i (between i-1 and i), duration = (i-1)+frac
            return (i - 1) + frac
    return None
...
# Streamlit UI
# -----------------------------
st.set_page_config(page_title="PV Payback (clear KPIs)", layout="wide")
...
st.write(
    ...
    "Enter your annual imported and exported energy, PV production (with optional degradation), "
    "battery round-trip losses, energy prices (with escalation), and maintenance costs. "
    "The app computes annual savings, cumulative cashflows, and when the initial invest is paid back."
)
...
def fmt_payback_year(y):
    return "No payback" if y is None else f"{int(round(y)):d}"

def fmt_payback_duration(y):
    return "No payback" if y is None else f"{y:.1f} years"
...
# -----------------------------
# Annual user cost (import – export + maintenance + inverter)
# grouped bars: Baseline vs Optimized
# -----------------------------
st.markdown("### Annual user cost (import – export + maintenance + inverter)")

def build_user_cost_df(df: pd.DataFrame, label: str) -> pd.DataFrame:
    # Import cost uses the escalated grid price per year
    import_cost = df["imported_kwh"] * df["grid_price_eur_kwh"]
    # Export credit uses the (constant) feed-in tariff per year
    export_credit = df["exported_kwh"] * df["feedin_eur_kwh"]
    annual_user_cost = import_cost - export_credit + df["opex_eur"] + df["inverter_cost_eur"]
...
st.altair_chart(bars.properties(height=340), use_container_width=True)
...
st.markdown("—")
st.caption("KPIs: Payback year (integer), duration to payback (years from t0), and Annual savings (Year 1).")
