...
class Params:
    ...
    base_fee_eur_year: float = 0.0   # annual base fee (EUR/year), applied to user costs, not to PV cashflow
...
def simulate_cashflows(p: Params) -> pd.DataFrame:
    ...
    # Annual user cost = Import cost − Export credit + OPEX + Inverter (+ Base fee)
...
# -----------------------------
# -----------------------------
def styled_axis(title: str, fmt="d", size_labels=14, size_title=16):
    return alt.Axis(
        title=title,
        format=fmt,
        labelFontSize=size_labels,
...
line = alt.Chart(plot_df).mark_line().encode(
    x=alt.X("calendar_year:Q", title="year", axis=alt.Axis(format="d"), labelFontSize=14, titleFontSize=16),
...
show_cols = [
    ...
    "avoided_import_eur", "base_fee_eur", "opex_eur", "inverter_cost_eur",
