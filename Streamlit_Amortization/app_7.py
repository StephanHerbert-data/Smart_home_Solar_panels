...
class Params:
    ...
    batt_charge_kwh_year: float      # Battery charge (kWh/yr)
    batt_discharge_kwh_year: float   # Battery discharge (kWh/yr)
...
def simulate_cashflows(p: Params) -> pd.DataFrame:
    ...
    Exported energy is derived from the energy balance:
    Exported = PV + Imported - Consumed - (BatteryCharge - BatteryDischarge).
    """
    ...
    batt_charge   = np.full_like(years, p.batt_charge_kwh_year, dtype=float)
    batt_discharge= np.full_like(years, p.batt_discharge_kwh_year, dtype=float)

    # Exported derived from balance; clamp to [0, pv_kwh]
    exported_raw = pv_kwh + imported_kwh - consumed_kwh - (batt_charge - batt_discharge)
    exported_kwh = np.clip(exported_raw, 0.0, pv_kwh)

    # Optional info metrics
    # Direct self-consumption (excluding battery): S = Consumed - Imported - BatteryDischarge
    self_consumed_direct = np.maximum(consumed_kwh - imported_kwh - batt_discharge, 0.0)

    # Avoided imports = Consumed - Imported (cannot be negative)
    avoided_import_kwh = np.maximum(consumed_kwh - imported_kwh, 0.0)

    # Cash components (nominal)
    avoided_import_cost = avoided_import_kwh * grid_price
    ...
    annual_user_cost  = import_cost_eur - export_credit_eur + opex + inverter_cost
    ...
    df = pd.DataFrame({
        ...
        "self_consumed_direct_kwh": self_consumed_direct,
        ...
        "annual_user_cost_eur": annual_user_cost,
        "opex_eur": opex,
...
# Streamlit UI
# -----------------------------
st.set_page_config(page_title="PV Payback (energy balance inputs)", layout="wide")
...
st.write(
    ...
    "Enter your annual imported and **consumed** energy, PV production (with optional degradation), "
    "and battery flows (charge & discharge). The app derives **exported energy** from the energy balance, "
    "applies grid price escalation, and computes annual savings, cumulative cashflows, "
    "annual user cost, and payback."
)
...
with st.sidebar:
    ...
    consumed_kwh = st.number_input("Consumed energy (kWh/yr)", value=4500.0, min_value=0.0, step=100.0)
    batt_charge_kwh = st.number_input("Battery charge (kWh/yr)", value=800.0, min_value=0.0, step=50.0)
    batt_discharge_kwh = st.number_input("Battery discharge (kWh/yr)", value=760.0, min_value=0.0, step=50.0)
...
shared = Params(
    ...
    batt_charge_kwh_year=batt_charge_kwh,
    batt_discharge_kwh_year=batt_discharge_kwh,
...
# -----------------------------
# NEW: Exported Energy line chart
# -----------------------------
...
show_cols = [
    ...
    "battery_charge_kwh", "battery_discharge_kwh", "exported_kwh",
...
st.markdown("—")
st.caption("Exported energy is derived from the energy balance. Annual user cost includes import costs with escalation, minus export credit, plus maintenance and inverter.")
