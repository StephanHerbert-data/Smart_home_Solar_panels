...
# Streamlit UI
# -----------------------------
st.set_page_config(page_title="PV Payback Simulator (Baseline vs Optimized)", layout="wide")

st.title("☀️ PV Payback: Baseline vs Optimized Price")
st.caption("Two scenarios with different grid prices. All other parameters are shared.")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("Prices")
    # Two independent grid prices for the two scenarios
...
# Build two scenarios: Baseline vs Optimized
...
# -----------------------------
# Chart: two lines for cumulative nominal cashflows
# -----------------------------
st.markdown("### Cumulative Cashflow (Nominal)")
plot_df = pd.DataFrame({
    "calendar_year": df_base["calendar_year"],
    "Baseline (cum €)": df_base["cum_cashflow_eur"],
    "Optimized (cum €)": df_opt["cum_cashflow_eur"],
}).set_index("calendar_year")
st.line_chart(plot_df, height=320)

# (Optional) If you also want discounted series comparison, uncomment:
# st.markdown("### Cumulative Discounted Cashflow")
# plot_dcf = pd.DataFrame({
#     "calendar_year": df_base["calendar_year"],
#     "Baseline (cum DCF €)": df_base["cum_dcf_eur"],
#     "Optimized (cum DCF €)": df_opt["cum_dcf_eur"],
# }).set_index("calendar_year")
# st.line_chart(plot_dcf, height=300)

# -----------------------------
# Annual details in tabs
# -----------------------------
...
st.markdown("—")
st.caption("Tip: Tweak only the grid prices to isolate tariff effects. All other inputs are shared across scenarios.")
